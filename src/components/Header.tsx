...
export const Header: React.FC<HeaderProps> = ({ onOpenCart, onOpenActiveOrder, onOpenHistory }) => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-xs w-full max-w-full overflow-hidden">
      {/* Top Banner / Ticker */}
      <div className="bg-slate-900 text-white text-xs px-3 sm:px-4 py-1.5 flex items-center justify-between gap-2 w-full max-w-full overflow-hidden">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <label className="flex items-center gap-1.5 text-[11px] sm:text-xs text-slate-300 cursor-pointer select-none">
            <input
              type="checkbox"
              checked={autoSimulateKitchen}
              onChange={(e) => setAutoSimulateKitchen(e.target.checked)}
              className="rounded border-slate-700 text-orange-500 focus:ring-0 w-3.5 h-3.5 accent-orange-500"
            />
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 w-full">
        <div className="flex items-center justify-between h-16 gap-2 sm:gap-4 min-w-0">
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 shrink">
            <div className="flex items-center gap-2 shrink-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] sm:text-[11px] text-slate-500 hidden lg:block font-medium">
            ...
            <div className="relative ml-1 sm:ml-2 shrink min-w-0">
              <button
                ...
                className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 rounded-full text-xs font-semibold bg-slate-100 hover:bg-slate-200/80 text-slate-800 border border-slate-200/80 transition max-w-[110px] sm:max-w-[160px] md:max-w-none"
              ...
              {/* Dropdown */}
              {showCanteenDropdown && (
                <div className="absolute left-0 mt-2 w-72 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 z-50">
                  ...
                  <div className="space-y-1 mt-1">
                    {canteens.map((c) => (
                      <button
                        ...
                        className={`w-full text-left p-2.5 rounded-xl text-xs flex items-start justify-between transition ${
                      ...
                      >
                        <div>
                          ...
                          <div className="text-[10px] text-slate-400 mt-0.5">{c.openingHours}</div>
                        </div>
                        <span
          ...
          <div className="hidden xl:flex items-center p-1 bg-slate-100 rounded-full border border-slate-200/80 shrink-0">
            {rolesList.map(({ role, label, icon: Icon }) => (
              ...
              >
                <Icon className={`w-3.5 h-3.5 ${currentRole === role ? 'text-orange-500' : 'text-slate-400'}`} />
                <span>{label}</span>
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
            ...
            <div className="relative shrink-0">
              <button
                ...
                className="p-1.5 sm:p-2 rounded-xl text-slate-600 hover:bg-slate-100 transition relative"
                title="Notifications"
              >
              ...
              {showNotifDropdown && (
                <div className="absolute right-0 mt-2 w-72 sm:w-96 bg-white rounded-2xl shadow-xl border border-slate-100 p-3 z-50">
                      ...
                      notifications.map((n) => (
                        <div
                          ...
                          className={`py-2.5 px-2 rounded-xl cursor-pointer transition ${
                            !n.read ? 'bg-orange-50/50' : 'opacity-80 hover:bg-slate-50'
                          }`}
                        >
            ...
            <div className="relative shrink-0">
              <button
                ...
                className="flex items-center gap-1.5 p-1 sm:px-2.5 sm:py-1.5 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-semibold transition"
              ...
              >
                ...
                <div className="text-left hidden md:block">
                  ...
                  <div className="text-[10px] text-slate-500 font-normal leading-none mt-0.5">
                ...
                <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 z-50">
                  ...
                  <div className="py-1">
                    <button
                      ...
                      className="w-full text-left px-3 py-2 text-xs text-slate-700 hover:bg-slate-50 rounded-xl flex items-center justify-between"
                    >
                  ...
                  <div className="text-[10px] font-semibold text-slate-400 px-3 py-1 uppercase border-t border-slate-100">
                    ...
                    {availableUsers.map((u) => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-1.5 text-xs rounded-xl transition flex items-center justify-between ${
                          u.id === currentUser.id ? 'bg-orange-50 font-bold text-orange-950' : 'hover:bg-slate-50 text-slate-700'
                        }`}
                      >
                        <div>
                          <div>{u.name}</div>
                          <div className="text-[10px] text-slate-400">{u.role === 'faculty' ? 'Faculty Member' : 'Student'}</div>
                        </div>
            ...
            {(currentRole === 'student' || currentRole === 'faculty') && (
              <button
                onClick={onOpenCart}
                className="relative flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-full bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs shadow-md shadow-orange-500/20 transition active:scale-95 shrink-0"
              >
                <ShoppingBag className="w-4 h-4" />
                <span className="hidden sm:inline">Cart</span>
                {cartCount > 0 && (
                  <span className="bg-slate-950 text-white text-[10px] font-extrabold w-4 h-4 rounded-full flex items-center justify-center -mr-0.5">
                    {cartCount}
                  </span>
                )}
              </button>
      ...
      <div className="xl:hidden bg-slate-100/90 border-t border-slate-200/80 px-2 sm:px-4 py-1.5 w-full">
        <div className="max-w-7xl mx-auto flex items-center justify-start sm:justify-center gap-1.5 overflow-x-auto no-scrollbar">
          {rolesList.map(({ role, label, icon: Icon }) => (
            <button
              key={role}
              onClick={() => setRole(role)}
              className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap transition shrink-0 ${
                currentRole === role
                  ? 'bg-orange-500 text-white shadow-xs'
                  : 'bg-white text-slate-700 hover:text-slate-900 border border-slate-200/80'
              }`}
            >
              <Icon className={`w-3.5 h-3.5 ${currentRole === role ? 'text-white' : 'text-slate-500'}`} />
              <span>{label}</span>
            </button>
          ))}
        </div>
      ...
      {showWalletModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-sm w-full p-5 shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">Fast 1-tap cashless pre-orders</p>
                </div>
              </div>
              <button
            ...
            <div className="mt-4">
              <label className="text-xs font-bold text-slate-700">Quick Recharge Amount</label>
            ...
            <div className="mt-5 flex gap-2">
              <button
                ...
                className="w-full py-2.5 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold flex items-center justify-center gap-1.5 shadow-md shadow-emerald-600/20 transition"
              >
